 ...
 * @author root
 */
class Set extends VersionedEntity{
    /**
     * Constructor
     * 
     * @param type $entityId
     ...
     */
    public function Set($entityId, $catalogId){
        $this->his_tableName = 'his_set';
        parent::VersionedEntity($entityId, $catalogId);
    }
    
    public function getHistoryEntity() {
        return $this->entity->hisSets[0];
        
    }
    public function getHistoryEntities() {
        return HisSet::model()->findAll('identifier_id=:entityId', array('entityId'=>$this->entity->id));
    ...
    protected function findEntity($entityId) {        
        ...
        $entity = CurrSet::model()->with('hisSets')->find('t.id=:entityId AND hisSets.catalog_id=:versionId',
    ...
    protected function updateHistoryRecord() {
        $hisRecord = $this->entity->hisSets[0];
    ...
    protected static function entityExistsCurr($model) {
       return CurrSet::model()->exists('id=:id', array(':id'=>$model->id));
    }
    protected static function entityExistsHis($model) {
        return HisSet::model()->exists('id=:id', array(':id'=>$model->id));
    }
